...
void main( ){ 
	...
	dummy_z.z = (uA * (cos(2*pi*uB*gl_Vertex.x+uC) * exp(-uD*gl_Vertex.x)) * (exp(-uE*gl_Vertex.y)));
	
	vec4 ECposition = gl_ModelViewMatrix * gl_Vertex;

	float dzdx = uA * (-sin(2*pi*uB*gl_Vertex.x+uC) * 2*pi*uB*exp(-uD*gl_Vertex.x) + cos(2*3.12*uB*gl_Vertex.x+uC) * -uD*exp(-uD*gl_Vertex.x)) * (exp(-uE*gl_Vertex.y));
	float dzdy = uA * (cos(2*pi*uB*gl_Vertex.x+uC) * exp(-uD*gl_Vertex.x)) * (-uE * exp(-uE*gl_Vertex.y));
